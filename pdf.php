...
class PDF extends FPDF
{
    public static function GenerarPDFPreventivo(
        ...
        // Título principal
        $pdf->Cell(0, 10, utf8_decode('Registro de asistencia / orden de trabajo Nro. Orden: '.$solicitudID.''), 0, 1, 'L');
        $pdf->Cell(0, 10, utf8_decode('Departamento Unidad: '.$departamento.'  Cod. Equipo: '.$solicitudID.''), 0, 1, 'L');
        ...
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(-78, 10, utf8_decode(''.$tipoSolicitud.''), 0, 0, 'R');
        $pdf->Ln(10);
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(0, 10, utf8_decode('Soporte: '), 0, 0, 'L');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(-69, 10, utf8_decode(''.$tipoMantenimientoString.''), 0, 0, 'R');
        $pdf->Ln(10);
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(0, 10, utf8_decode('Impresora Funciona: '), 0, 0, 'L');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(-54, 10, utf8_decode(''.$impresoraString.''), 0, 0, 'R');
